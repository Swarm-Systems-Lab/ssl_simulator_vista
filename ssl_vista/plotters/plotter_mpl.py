...
from ._base_plotters import BasePlotter
...
class BaseMplPlotter(BasePlotter):
    ...
    def get_artists_list(self):
        """Return the flattened list of all artist objects."""
        ...
        return list(flatten_artists(self._artists))
    ...
    def debug_artists(self):
        ...
        """
        print("----- BaseMplPlotter Artists Debug -----")
        total_count = 0
        ...
        for ax_key, ax in self.axes.items():
            ...
            group = self._artists.get(ax_key, None)
            if not group:
