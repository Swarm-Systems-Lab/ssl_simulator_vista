...
class Plotter2DCanvas(BaseCanvasPlotter):
    ...
    def _check_data_shapes(self, sim_data):
        ...
        data_heading = sim_data.get(self.label_heading)
        if data_heading is not None and (
            data_heading.ndim != 2 or data_heading.shape[1] != data_pos.shape[1]
        ):
            raise ValueError(
                f"Heading array must be shape (T,N={data_pos.shape[1]}), got {data_heading.shape}"
            )
